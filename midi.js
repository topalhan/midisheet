  ...
  handleMidiMessage(msg, deviceName = 'MIDI In') {
    ...
    if (command === 9) {
      if (velocity > 0) {
        ...
        this.logMessage('Note On', deviceName, `Note ${note}, Vel ${velocity}`, data);
      } else {
        ...
        this.logMessage('Note Off', deviceName, `Note ${note}`, data);
      }
    } else if (command === 8) {
      // Note Off (0x80)
      const state = this.noteStates.get(note);
      ...
      if (ccNumber === 64) {
        ...
        this.logMessage('Sustain Pedal', deviceName, isDown ? 'Pedal DOWN' : 'Pedal UP', data);
        ...
        this.logMessage('Breath/Exp CC', deviceName, `CC #${ccNumber}: ${ccValue}`, data);
      ...
      this.logMessage('Aftertouch', deviceName, `Pressure: ${pressureValue}`, data);
    } else if (command === 10) {
      // Polyphonic Aftertouch (0xA0)
      const pressureValue = velocity; // data[2]
      const pressureNorm = pressureValue / 127;
      if (this.onBreath) this.onBreath(pressureNorm, 'polyAftertouch');
      this.logMessage('Poly Pressure', deviceName, `Note ${note}: ${pressureValue}`, data);
    }
  }

  logMessage(type, source, detail, rawBytes) {
